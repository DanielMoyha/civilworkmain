...
<div>
    <div>
        ...
        <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)">
            <div x-init="$notification({ text: 'Rol eliminado correctamente!', variant: 'success', position: 'right-top', duration: 2200 })"></div>
        </p>
        <?php endif; ?>
        <?php if(session('status') === 'cannot-deleted'): ?>
        <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3500)">
            <div x-init="$notification({ text: 'Este rol es ocupado por uno o más usuarios, no se puede eliminar!', variant: 'warning', position: 'right-top', duration: 3500 })"></div>
        </p>
        <?php endif; ?>
    </div>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 lg:gap-6">
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <p><?php echo e(__('Sin Roles registrado aún')); ?></p>
        <?php endif; ?>
    </div>
</div>

            ...
            <table class="is-hoverable w-full text-left">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $allconstructions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $construction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="<?php if($construction->work->status === 0): ?> bg-error/15 hover:!border-error hover:text-error <?php endif; ?> border border-transparent border-b-slate-200 dark:border-b-navy-500" wire:loading.class='opacity-40'>
                            <td class="text-center px-4 py-3 sm:px-5"><?php echo e($loop->iteration); ?></td>
                            <?php if($construction->work->status === 0): ?>
                            ...
                            <?php else: ?>
                                <?php if($construction->work->completion_date): ?>
                                    <td class=" px-4 py-3 sm:px-5">
                                        ...
                                        <a href="<?php echo e(route('admin.works.show', [$construction->work->id])); ?>">
                                            <?php echo e($construction->name); ?>

                                        </a>
                                    </td>
                                <?php else: ?>
                                    <?php if($construction->work->user->is_active !== 1): ?>
                                        <td class="px-4 py-3 sm:px-5">
                            ...
                            <?php if($construction->work->status === 0): ?>
                                <td class="dark:text-white decoration-double hover:text-error px-4 py-3 sm:px-5 text-base text-center tracking-wider underline uppercase" colspan="2">
                                    <i class="fa-solid fa-circle-info text-info" x-tooltip.info.on.mouseenter="'Se dio de baja en fecha: <?php echo e($construction->work->updated_at->format('d-m-Y')); ?>'"></i> <?php echo e(__('OBRA DADA DE BAJA')); ?>

                                </td>
                            <?php else: ?>
                                <?php if($construction->materials()->exists()): ?>
                                    <td class="text-center text-xs+ px-4 py-3 sm:px-5">
                                        <?php echo e($construction->materials->count() . __(' Tipos de materiales utilizados hasta la fecha')); ?>

                                    </td>
                                <?php else: ?>
                                    <td class="text-center font-bold text-xs italic opacity-40 px-4 py-3 sm:px-5">
                                        <?php echo e($construction->materials->count() . __(' materiales')); ?>
                            ...
                            <td colspan="4" class=" px-4 py-3 sm:px-5">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <span class="font-medium py-8 text-cool-gray-400 text-xl">
                                        <?php echo e(__('Construcciones no encontradas...')); ?>

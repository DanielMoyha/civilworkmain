<div>
    <p class="text-base font-medium text-slate-600 dark:text-navy-50 mb-1">{{ __('Tareas Completadas') }}</p>
            ...
            <div class="border-b border-slate-150 py-3 dark:border-navy-500">
                ...
                <div class="mt-1 flex items-end justify-between">
                    <div class="flex flex-wrap items-center font-inter text-xs">
                        <p>{{ $task->completed_at->diffForHumans() }}</p>
                        ...
                        <span class="flex items-center space-x-1">
                            ...
                            <span>{{ $task->completed_at->format('H:i') }}</span>
                        </span>
                        <div class="m-1.5 w-px self-stretch bg-slate-200 dark:bg-navy-500"></div>
                    ...
                    <div class="flex items-center space-x-1">
                        <button x-data="{isImportant:false}" @click.stop="isImportant =! isImportant" class="btn h-7 w-7 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25" wire:click="returnTask({{$task->id}})">
        ...
        @empty
            <div class="content-center grid h-full justify-center">
                <h2 class="text-sm+ font-medium tracking-wide text-slate-700 line-clamp-1 dark:text-navy-100 mt-2">
                    <i class="fa-solid fa-inbox"></i> {{ __('Aún no tiene tareas completadas') }}
                </h2>
            </div>
        @endforelse
    </div>

                ...
                <tbody>
                    @forelse ($allServices as $service)
                        <tr class="border border-transparent border-b-slate-200 dark:border-b-navy-500" wire:loading.class='opacity-40'>
                            ...
                            <td>{{ $service->name }}</td>
                            <td class="text-center"><i class="fa-solid fa-calendar-days"></i> {{ $service->created_at->translatedFormat('j F Y') }}</td>
                            <td class="flex justify-center items-center gap-2 py-2">
                                <button wire:click="edit({{ $service->id }})" class="btn bg-warning font-medium text-white hover:bg-warning-focus focus:bg-warning-focus active:bg-warning-focus/90 h-8 w-1"><i
                                        class="fa-solid fa-pen-to-square fa-lg"></i></button>
                                <button wire:click="$emit('showAlert', {{ $service->id }})" class="btn bg-error font-medium text-white hover:bg-error-focus focus:bg-error-focus active:bg-error-focus/90 h-8 w-1"><i
                        ...
                        <tr>
                            <td colspan="4">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <span class="font-medium py-8 text-cool-gray-400 text-xl">{{ __('Servicios no encontrados...') }}</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse

...
<x-main class="is-sidebar-open">
    ...
    @section('content')
        <main class="main-content w-full px-[var(--margin-x)] pb-8">
            <div class="flex items-center space-x-4 py-5 lg:py-3">
                {{-- <h2 class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl"></h2> --}}
            </div>
                    ...
                    <div class="card">
                        <div class="border-b border-slate-200 p-4 dark:border-navy-500 sm:px-5">
                            <div class="flex items-center space-x-2">
                                ...
                                <h4 class="text-lg font-medium text-slate-700 dark:text-navy-100">Actualización de datos del Rol</h4>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4 p-4 sm:p-5">
                                <label class="block">
                                    ...
                                    <input
                                        ...
                                        placeholder="Ingrese el nombre del Rol" type="text" name="name" id="name" value="{{ $role->name }}"/>
                                    @error('name')
                                ...
                                <label class="block">
                                    ...
                                    <input
                                        class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                        placeholder="Describa brevemente el rol" type="text" name="description" id="description" value="{{ $role->description }}"/>
                                    @error('description')
                                        <small class="text-danger">
                                            {{ $message }}
                                        </small>
                                        ...
                                        @foreach ($permissions as $permission)
                                            <label class="inline-flex items-center space-x-2">
                                                <input
                                                    ...
                                                    type="checkbox"
                                                    name="permissions[]"
                                                    value="{{ $permission->id }}" {{ in_array($permission->id, $userHasRoles) ? 'checked' : '' }}
                                                    id="{{ $permission->id }}"
                                                />
